...
class KFCSpider:
    def get_coupons(self):
        ...
        # 產生網址
        for i in range(1, 11):
            link_1 = 'https://goodlife.tw/%E9%80%9F%E9%A3%9F%E5%BA%97/%E8%82%AF%E5%BE%B7%E5%9F%BA/page/'
            link_1 = link_1 + str(i)
            ...
            for i in tmp[1:]:
                ...
                if '(已過期)' not in name and postTime.find('2018') != -1:  # 取未過期的 and postYear 2018
                    ...
                    # 爬 折價卷照片
                    img = coupon_articles.findAll(
                        'img')  # .get('src') #get 圖片--okokokokokokokokokokokokokokokokokokokokokokokokokokokokokkoko
                    # print('img:')
                    imgList = []
                    for i in img:
                        img = i.get('src')
